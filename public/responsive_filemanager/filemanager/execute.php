		...
		case 'rename_file':
			if ($config['rename_files']){
				...
				if (!empty($name))
				{
                    ...
                    foreach ($config['multiple_thumb'] as $k=>$v){
                        $path_parts = pathinfo($path_thumb);
                        $mt_path = $path_parts['dirname'].'/'.$path_parts['filename'].'-'.$v['width'].'x'.$v['height'].'.'.$path_parts['extension'];
                        $new_mt_name = $name.'-'.$v['width'].'x'.$v['height'];
                        rename_file($mt_path,$new_mt_name,$ftp,$config);
                    }
					if ($config['fixed_image_creation'])
					{
						$info=pathinfo($path);
