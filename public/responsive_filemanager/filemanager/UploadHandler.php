...
class UploadHandler
{
	...
	public function onUploadEnd ($res){
		...
		if ($is_img)
		{
            ...
            //generate more thumbs
            foreach ($this->options['config']['multiple_thumb'] as $k=>$v){

                $path_parts = pathinfo($targetFileThumb);
                $new_mt_path = $path_parts['dirname'].'/'.$path_parts['filename'].'-'.$v['width'].'x'.$v['height'].'.'.$path_parts['extension'];
                $mt = create_img($targetFile, $new_mt_path, $v['width'], $v['height']);
            }
